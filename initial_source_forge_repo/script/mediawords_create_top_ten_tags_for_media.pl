...
sub main
{
    ...
    my $top_ten_for_media_id_query =
        'select * from tags, media, '
      ...
      . " where foo.tags_id=tags.tags_id and foo.media_id=media.media_id and  not (media.name ilike '%' || (regexp_replace(tag, '^the ', '')) || '%') limit 10 ";
...
"CREATE TABLE $temp_table_name (  media_id integer not null references media on delete cascade, tags_id integer not null references tags,     media_tag_count integer not null,     tag_name character varying(512) not null,     tag_sets_id integer not null references tag_sets) "
    ...
    foreach my $media_id ( sort { $a <=> $b } @media_ids )
    {
        ...
        foreach my $tag_set_id ( sort { $a <=> $b } @tag_set_ids )
        {
            ...
            my $result = $db->query( $top_ten_for_media_id_query, $media_id, $tag_set_id );
    ...
    foreach my $row_to_insert (@rows_to_insert)
    {
        ...
        $db->query(
            ...
            $row_to_insert->{media_id}, $row_to_insert->{tags_id}, $row_to_insert->{media_tag_count},
            $row_to_insert->{tag},      $row_to_insert->{tag_sets_id}
        );
    }
    ...
    $db->query_only_warn_on_error("create index media_id_and_tag_sets_id_index on $temp_table_name (media_id,tag_sets_id)");
