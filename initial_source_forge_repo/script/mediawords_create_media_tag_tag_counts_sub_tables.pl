...
sub get_rows_in_stories_id_window
{
    ...
    my $rows = $dbh->query(
"select media_id, publish_date, stories_tags_map.* from (select media_id, publish_date, stories_id from stories where stories.stories_id < ? and stories.stories_id >= ? and stories.publish_date > (now() - interval ' 90 days')) as stories, stories_tags_map where stories.stories_id=stories_tags_map.stories_id   and stories_tags_map.stories_id < ? and stories_tags_map.stories_id >= ?  order by stories.stories_id",
...

{
    ...
    while ( $_stories_id_start <= $max_stories_id )
    {
        ...
        $rows_to_insert->bind( my ( $media_id, $publish_date, $stories_tag_map_id, $stories_id, $tags_id ) );
        ...
        while ( $rows_to_insert->fetch )
        {
            ...
            $dbh->query( 'INSERT INTO '
                  . get_sub_table_name_for_media_id($media_id)
                  . '  (media_id, publish_date, stories_id, tags_id) VALUES (?,?,?, ?)',
                $media_id, $publish_date, $stories_id, $tags_id );
        }
