...
sub main {
    ...
    my ( $last_stories_id ) = $db->query( 
        "select first_stories_id from story_sentence_counts " .
        "  where story_sentence_counts_id = 0" )->flat;
    ...
    if ( $start_date )
    {
        $stories_query = "select * from stories where stories_id > ? " . 
    ...
    while ( my $stories = $db->query( $stories_query, $last_stories_id )->hashes )
    {
        ...
        {
            my $sentences = $db->query( "select * from story_sentences where stories_id = ?", $story->{ stories_id } )->hashes;
            for my $sentence ( @{ $sentences } )
            {
                $total_sentence_count++;
                my $num_dups = count_duplicate_sentences( 
                    $db, $sentence->{ sentence }, $sentence->{ sentence_number }, $story );
                    
                if ( $num_dups > 0 )
                {
                    $db->query( 
                        "delete from story_sentences where stories_id = ? and sentence_number = ?",
                        $sentence->{ stories_id }, $sentence->{ sentence_number } );
                    $db->query( 
        ...
        $db->create( 'story_sentence_counts', 
            { sentence_md5 => 'na', media_id => 0, publish_week => '2010-01-01', sentence_count => 0, first_sentence_number => 0, 
              first_stories_id => $last_stories_id, story_sentence_counts_id => 0 } );              
