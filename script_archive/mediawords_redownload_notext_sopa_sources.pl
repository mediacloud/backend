...
sub redownload_download
{
    ...
    my ( $new_length ) =
      $db->query( "select download_text_length from download_texts where downloads_id = ?", $download->{ downloads_id } )
      ->flat;
...
END
    ...
    $db->query(
        "delete from story_similarities where stories_id_a = ? or stories_id_b = ?",
        $story->{ stories_id },
        $story->{ stories_id }
    );
...
sub fix_notext_medium
{
    ...
    print
"fixing medium $medium->{ name } [ $medium->{ media_id } $medium->{ average_story_length } $medium->{ story_count } ] ...\n";
