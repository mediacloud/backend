...
sub main
{
    ...
    my $queries = $db->query( "select * from queries where dashboards_id is null or md5_signature is null order by queries_id" )->hashes;
    
    my $i = 1;
    for my $query ( @{ $queries } )
    {
        print STDERR "progress: " . $i++ . " / " . @{ $queries } . " \n" unless ( $i % 100 );
        $query->{ media_sets_ids } = $db->query( 
            "select media_sets_id from queries_media_sets_map where queries_id = ?",
            $query->{ queries_id } )->flat;
        $query->{ dashboard_topics_ids } = $db->query( 
            "select dashboard_topics_id from queries_dashboard_topics_map where queries_id = ?",
            $query->{ queries_id } )->flat;
