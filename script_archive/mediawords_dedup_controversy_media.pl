...
sub dedup_stories
{
    ...
    # replace with simple delete from stories if references are cascading correctly
...
sub merge_into_medium
{
    ...
    for my $source_medium ( @{ $source_media } )
    {
        ...
        $db->query(
            "insert into controversy_merged_media ( source_media_id, target_media_id ) values ( ?, ? )",
            $source_medium->{ media_id },
            $target_medium->{ media_id }
        );

        $db->query(
            "update feeds set media_id = ? where media_id = ?",
            $target_medium->{ media_id },
            $source_medium->{ media_id }
        );
...
END
    ...
    my $media = $db->query( $query, $spidered_tag->{ tags_id }, $controversies_id )->hashes;
    ...
    while ( my ( $domain, $domain_media ) = each( %{ $media_domain_lookup } ) )
    {
        ...
        next if ( $resume_media_id && ( $domain ne $resume_media_id ) );

        $resume_media_id = undef;
