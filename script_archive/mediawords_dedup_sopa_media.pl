...
sub dedup_stories
{
    ...
    for my $dup_story ( @{ $dup_stories } )
    {
        ...
        $db->query(
            "delete from sopa_links where stories_id = ? or ref_stories_id = ?",
...
sub merge_into_medium
{
    ...
    for my $source_medium ( @{ $source_media } )
    {
        ...
        $db->query(
            "insert into sopa_merged_media ( source_media_id, target_media_id ) values ( ?, ? )",
            $source_medium->{ media_id },
            $target_medium->{ media_id }
        );
...
sub main
{
    ...
    my $spidered_tag = MediaWords::Util::Tags::lookup_tag( $db, 'spidered:sopa' )
      || die( "Unable to find spidered:sopa tag" );
