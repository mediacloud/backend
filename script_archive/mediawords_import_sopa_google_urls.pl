...
sub find_or_create_search_medium
{
    ...
    my $medium = $db->query( 
        "select m.*, f.feeds_id from media m, feeds f where m.media_id = f.media_id and m.name = ?", SOPA_GOOGLE_SEARCH_NAME )->hash;
...
sub get_search_story
{
    ...
    my $story = $db->query( 
        "select * from stories where media_id = ? and url = ?", 
        $medium->{ media_id }, $story_url )->hash;
...
sub import_link
{
    ...
    my $link = $db->query( "select * from sopa_links where stories_id = ? and url = ?", $story->{ stories_id }, $enc_url )->hash;
