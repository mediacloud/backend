...
sub get_shows_from_csv
{
    ...
    my $csv = Text::CSV_XS->new( { binary => 1, sep_char => "\t" } ) || die "error using CSV_XS: " . Text::CSV_XS->error_diag();
...
sub find_or_create_story
{
    ...
    my $story = $db->query( "select * from stories where media_id = ? and guid = ?", $medium->{ media_id }, $story_url )->hash;
    
    return $story if ( $story );
    
    print STDERR "create story $story_url / $story_title\n";
    
    my $story = $db->create( 'stories', 
        {
            media_id => $medium->{ media_id },
            url => $story_url,
            guid => $story_url,
    ...
    $db->query( "insert into feeds_stories_map ( feeds_id, stories_id ) values ( ?, ? )", $feed->{ feeds_id }, $story->{ stories_id } );
...
sub find_or_create_download_text
{
    ...
    my $download_text = $db->query( "select * from download_texts where downloads_id = ?", $download->{ downloads_id } )->hash;
    ...
    $download_text = $db->create( 'download_texts',
        {
...
sub import_show
{
    ...
    print STDERR "import show complete: s $story->{ stories_id } / d $download->{ downloads_id } / dt $download_text->{ download_texts_id }\n";    
}
