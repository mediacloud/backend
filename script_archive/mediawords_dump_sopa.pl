...
sub write_media_text_links
{
    ...
    write_dump_as_csv( $db, 'sopa_media_text_links_dump',
        ...
        "    s_a.media_id = m_a.media_id and s_b.media_id = m_b.media_id " .
        "  group by m_a.media_id, m_b.media_id " );
}
...
sub get_textlink_weighted_edges
{
    ...
    my ( $media_text_links_sum ) = $db->query( "select sum( similarity_sum ) from sopa_media_text_links_dump_${ dump_version } " )->flat;
...
sub write_gexf_dump
{
    ...
    my $media = $db->query( 
        "select smd.* from sopa_media_dump_${ dump_version } smd order by link_count desc" )->hashes;
    my $stories = $db->query( 
...
sub main 
{
    ...
    Getopt::Long::GetOptions(
        ...
        "end_date=s" => \$end_date,
        "period=s" => \$period,
        "weighting=s" => \$weighting ) || return;
        
    die( "Usage: $0 --start_date < start date > --end_date < end date > --period < overall|weekly|monthly|all > --weighting < link|text|text+link >" )
