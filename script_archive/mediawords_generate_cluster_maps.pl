...
sub get_cluster_run
{
    ...
    my $cluster_run =
      $db->query( "select * from media_cluster_runs " . "  where queries_id = ? and clustering_engine = 'media_sets' ",
        $query->{ queries_id } )->hash;

    return $cluster_run if ( $cluster_run && $cluster_run->{ state } eq 'completed' );

    my $cluster_run ||= $db->create(
        'media_cluster_runs',
        {
...
sub get_cluster_map
{
    ...
    return MediaWords::Cluster::Map::generate_cluster_map( $db, $cluster_run, 'polar', [ $polar_query ], 1,
        'graphviz-neato' );
}
...
sub print_cluster_map_link
{
    ...
    if ( $topic_set )
    {
        ...
        $topic_label = join( ',',
            $db->query( "select name from dashboard_topics " . "where dashboard_topics_id in ($topics_ids_list)" )->flat );
    }
