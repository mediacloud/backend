...
sub get_sopa_stories
{
    ...
    return $db->query( "select distinct s.*, m.name as media_name from stories s, sopa_stories ss, media m " .
...
sub generate_similarities
{
    ...
    for my $story ( @{ $stories } )
    {
        ...
        $db->dbh->do(
            "copy story_similarities( stories_id_a, publish_day_a, stories_id_b, publish_day_b, similarity ) from STDIN" );
        ...
        for ( my $i ; $i < @{ $story->{ similarities } } ; $i++ )
        {
            ...
            my $put = join( "\t",
                ...
                $story->{ publish_date },
                $story_b->{ stories_id },
                $story_b->{ publish_date },
                int( $sim * SIMILARITY_SCALE ) );
...
sub get_most_similar_stories
{
    ...
    for my $story ( @{ $stories } )
    {
        ...
        my $count_a = $db->query(
            ...
            $story->{ stories_id },
            SIMILARITY_THRESHOLD * SIMILARITY_SCALE
        )->list;
        my $count_b = $db->query(
            "select count(*) from ( " .
        ...
        $story_sim_counts->{ $story->{ stories_id } } = { count => $count, story => $story };
    }
    print STDERR "\n";
...
sub print_similar_stories_csv
{
    ...
    for my $similar_story ( @{ $similar_stories } )
    {
        ...
        print STDERR
"$similar_story->{ count }: [ $story->{ stories_id } ] $story->{ title } ( $story->{ media_name } : $story->{ publish_date } )\n";
    ...
    my $encoded_csv =
      MediaWords::Util::CSV::get_hashes_as_encoded_csv( $similar_stories_hashes,
        [ qw(first_stories_id sim_count stories_id title url publish_date media_id media_name) ] );
