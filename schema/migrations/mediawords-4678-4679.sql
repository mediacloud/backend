...
-- versions 4678 and 4679.
--
-- If you are running Media Cloud with a database that was set up with a schema version
-- 4678, and you would like to upgrade both the Media Cloud and the
-- database to be at version 4679, import this SQL file:
--
--     psql mediacloud < mediawords-4678-4679.sql
--
...
-- NULLs allowed and no DEFAULT value to minimize AccessExclusiveLock duration
ALTER TABLE feeds ADD COLUMN active BOOLEAN NULL;
ALTER TABLE feeds_from_yesterday ADD COLUMN active BOOLEAN NULL;

-- Treat all feeds with "inactive" / "skipped" feed_status as inactive
UPDATE feeds SET active = CASE WHEN feed_status = 'active' THEN true ELSE false END;
UPDATE feeds_from_yesterday SET active = CASE WHEN feed_status = 'active' THEN true ELSE false END;

-- Readd NOT NULL
ALTER TABLE feeds ALTER COLUMN active SET NOT NULL;
ALTER TABLE feeds_from_yesterday ALTER COLUMN active SET NOT NULL;

-- Readd DEFAULT value ("feeds_from_yesterday" doesn't have it because it stores rows copied from "feeds")
ALTER TABLE feeds ALTER COLUMN active SET DEFAULT 't';

-- DROP feed_status
ALTER TABLE feeds DROP COLUMN feed_status;
ALTER TABLE feeds_from_yesterday DROP COLUMN feed_status;

-- DROP the now-unused type
DROP TYPE feed_feed_status;
