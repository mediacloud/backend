...
# With so many shards in place we need to do autovacuum more often
MC_POSTGRESQL_AUTOVACUUM_MAX_WORKERS=$((MC_CPU_COUNT / 3))
...
if (( $MC_POSTGRESQL_AUTOVACUUM_MAX_WORKERS < 3 )); then
    MC_POSTGRESQL_AUTOVACUUM_MAX_WORKERS=3
fi
...
autovacuum_max_workers = ${MC_POSTGRESQL_AUTOVACUUM_MAX_WORKERS}
EOF
