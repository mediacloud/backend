...
sub get_stem 
{
    ...
    my $stems = Lingua::Stem::En::stem({ -words => [ $query ],
                                         -locale => 'en' });
    if (!@{$stems}) {
...
sub get_tag {
    ...
    my $tag = $_db->query("select t.* from tags t, tag_sets ts where t.tag_sets_id = ts.tag_sets_id and " .
                          "    t.tag = ? and ts.name = ?", $tag_name, $tag_set_name)->hash;
    if (!$tag) {
...
sub get_sources {
    ...
    if (($type eq 'feeds') || ($type eq 'media')) {
        return $_db->query("select ?::text as type, name, ${type}_id as id " .
        ...
        for my $tag ( split(/\s+/, $id) ) {
            ...
            push(@{$sources}, $_db->query("select 'tags' as type, t.tag as name, t.tags_id as id from tags t, tag_sets ts " . 
                                          "  where ts.tag_sets_id = t.tag_sets_id and ts.name = ? and t.tag = ?",
                                          $tag_set_name, $tag_name)->hash);
        }
...
sub get_pr {
    ...
    my $words = $_db->query("select $pr_equation as p, a.term, a.stem $a_time_slice_field, a.p as a_p, b.p as b_p " .
...
sub get_p {
    ...
    my $words = $_db->query("select least( $max_p, avg(term_count / source_sentence_count) ) as p, stem, " . 
                            "    min(term) as term $time_slice_field from $table " . 
...
sub generate_timeline_chart_url {
    ...
    push(@{$params}, 'chd=s:' . join(',', map { join('', map { $enc->{ int($_ * ( 60 / $max))} } @{$_}) } @{$time_slicely_data}));    
...
sub get_pr_file {
    ...
    my $name = join( '_', 'pr', $source->{type}, $source->{id}, $source->{name},
                     $pr_source->{type}, $pr_source->{id}, $pr_source->{name} ) . ".html";
...
sub generate_source_page {
    ...
    $pr_sources = [ grep { !( ( $_->{type} eq $source->{type} ) && ( $_->{id} eq $source->{id} ) ) } @{$pr_sources} ];
...
sub insert_query_sentences {
    ...
    $_db->query("insert into query_sentences " . 
                "  (stories_id, sentence_number, media_id, feeds_id, time_slice) " . 
                "  select distinct topics.stories_id, topics.sentence_number, s.media_id, fsm.feeds_id,  " .
                "    cast(date_trunc('day', s.publish_date) as date) as time_slice " .
                "  from stories s left join feeds_stories_map fsm on ($type_join), " .
                "    story_sentence_words topics, media m " .
                "  where s.stories_id = topics.stories_id and s.media_id = m.media_id " .
                "    and s.publish_date >= date '$start_date' and s.publish_date <= date '$end_date' " .
                "    and topics.stem = lower(?)  " .
                "    and $type_clause", $stem);
...
sub insert_query_words 
{
    ...
    $_db->query("insert into query_words " . 
                ...
                "    " . MIN_SOURCE_SENTENCES . "::numeric as source_sentence_count, " .
                "    qs.media_id, qs.feeds_id, qs.time_slice " . 
                ...
                "  group by qs.media_id, qs.feeds_id, time_slice, sw.stem", $stem);
}
...
{                
    $_db->query("create temporary table all_query_words " .
                "  as select stem, min(term) as term from query_words group by stem");
    ...
    $_db->query("create temporary table query_words_source as " .
                "  select term, stem, 0::numeric as term_count, " . 
                "      " . MIN_SOURCE_SENTENCES . "::numeric as source_sentence_count, " .
                "      media_id, feeds_id " .
                "    from all_query_words, all_query_sources");

    $_db->query("update query_words_source as qws " . 
    ...
    $_db->query("update query_words_source as qw set source_sentence_count = q.sum_sentence_count " .
                "  from (select qs.media_id, qs.feeds_id, " . 
                "        greatest( " . MIN_SOURCE_SENTENCES . ", " . 
                "                  count(distinct qs.stories_id::text || 'Z' || qs.sentence_number::text) ) " . 
                "          as sum_sentence_count " .
                "      from query_sentences qs " .
...
sub generate_reports_from_db {
    ...
    while (1) {
        ...
        my $topics = $_db->query("select * from word_cloud_topics where state = 'pending' order by word_cloud_topics_id")->hashes;
        for my $topic (@{$topics}) {
            ...
            generate_report($topic_dir, 'story_' . $topic->{type}, $topic->{source_tags_id}, $topic->{set_tag_names}, 
