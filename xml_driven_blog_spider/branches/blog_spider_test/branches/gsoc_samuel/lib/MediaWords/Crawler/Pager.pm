...
#             title="Next Page"
#             href="/2008/03/20/garden/20dressing.html?pagewanted=2&ei=5088&en=66294369293563de&ex=1363665600&partner=rssnyt&emc=rss">Next Page &#x00bb;</a>
...
# use a variety of texts to guess whether a given link is
# a link to a next page
...
# look for a 'next page' link in the give html content and
# return the associated link if found
sub get_next_page_url
{

    #my ($class, $rs, $url, $html) = @_

    my $download_rs = $_[1];
    my $base_url    = $_[2];
    my $content_ref = \$_[3];

    # blogs and forums almost never have paging
    ...
    for ( my $i = 0 ; $i < $content_length ; $i++ )
    {
        if ( substr( $$content_ref, $i, 3 ) =~ /<a\s/is )
        {
            ...
            if ( _link_is_next_page( $raw_url, $full_url, $text, $base_url )
                && !$download_rs->find( { url => $full_url } ) )
            {
                $url = $full_url;
