...
sub view : Local
{
    ...
    my @downloads = $c->dbis->query( "select d.* from downloads d where d.type = 'content' " . 
                                     "    and d.state = 'success' and d.stories_id = ?", $stories_id )->hashes;
...
sub add_do : Local
{
    ...
    $c->response->redirect(
        $c->uri_for( '/stories/add/' . $story->{stories_id}, { status_msg => 'Tag \'' . $added_tag->{tag} . '\' added.' } )
    );
}
