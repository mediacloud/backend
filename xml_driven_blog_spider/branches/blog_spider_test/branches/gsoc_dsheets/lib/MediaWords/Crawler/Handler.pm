...
sub _convert_to_dbix_class_to_dbix_simple
{
    my ($row) = @_;
    ...
    for my $item ( @{$items} )
    {
        ...
        if ( !$story )
        {
            ...
            $story = $dbs->query( "select * from stories where title = ? and media_id = ? " . 
                                  ...
                                  decode( 'utf-8', $item->title ), $media_id )->hash;                                     
        }
...
sub _call_pager
{
    ...
    if ( my $next_page_url = MediaWords::Crawler::Pager->get_next_page_url( $validate_url, $download->{url}, $response->content ) )
    {
