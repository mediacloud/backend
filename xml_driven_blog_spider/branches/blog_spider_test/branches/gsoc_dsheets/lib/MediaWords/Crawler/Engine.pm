...
sub _run_fetcher
{
    ...
    while (1)
    {
        ...
        eval {
            ...
            print STDERR "fetcher " . $self->fetcher_number . " "
...
sub spawn_fetchers
{
    ...
    {
        #sharing dbs handles between processes doesn't work well so
        #get rid of the current dbs handle before we fork
        $self->close_and_undefine_dbs();

        my ( $parent_socket, $child_socket )
...
sub dbs
{
    ...
    if ( defined( $dbs ) )
    {
        ...
        defined( $self->{dbs} ) || die "error opening database connection $@";
        if ( !( $self->{dbs}->{sucess} ) && ( $self->{dbs}->{reason} ) )
        {
            die $self->{dbs}->{reason};
        }
    }
