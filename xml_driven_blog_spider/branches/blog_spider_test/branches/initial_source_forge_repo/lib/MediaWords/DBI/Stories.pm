...
sub get_text
{
    ...
    return join( "\n***\n\n", _strip_html($title), _strip_html($description), @{$download_texts} );
}
...
sub _add_module_tags
{
    ...
    $db->dbh->do("copy $subtable_name (media_id, publish_date, stories_id, tags_id) from STDIN");
    for my $tags_id (@tags_ids)
    {
        my $put_statement = join( "\t", $media_id, $story->{publish_date}, $story->{stories_id}, $tags_id ) . "\n";
