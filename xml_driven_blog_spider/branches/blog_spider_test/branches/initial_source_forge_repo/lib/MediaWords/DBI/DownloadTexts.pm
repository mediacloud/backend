...
sub create_from_download 
{
    ...
    my $download_text = $db->create( 'download_texts', { download_text => encode( 'utf-8', $extract->{extracted_text} ), 
    ...
    # FIXME - remove once all stories have been added to story_vectors and we can start calling
    # update_pending_story_vectors -hal
    $db->query( "delete from story_vectors where stories_id = ?", $download->{stories_id});
    $db->query( "delete from story_words where stories_id = ?", $download->{stories_id});
