...
sub list : Local
{
    ...
    my $topics = $c->dbis->query("select w.*, t.tag as source_tag_name " . 
...
sub create_do : Local
{
    ...
    if ($start_date !~ /^\d\d\d\d-\d\d-\d\d$/) {
        $c->response->redirect( $c->uri_for( "/topics/list/", { error_msg => 'start_date must be in the form YYYY-MM-DD' } ) );
        return;
    }        
    ...
    my ($source_tags_id) = $c->dbis->query("select t.tags_id from tags t, tag_sets ts " .
                                           "  where t.tag_sets_id = ts.tag_sets_id " . 
                                           "    and t.tag = ? and ts.name = 'word_cloud'",
                                           $source_tag_name)->flat;
    if (!$source_tags_id) {
    ...
    for my $set_tag_name (split(/[,\s]+/, $set_tag_names)) {
        ...
        my $tag_exists = $c->dbis->query("select 1 from tags t, tag_sets ts where t.tag_sets_id = ts.tag_sets_id " .
                                         "    and t.tag = ? and ts.name = ?", $tag, $tag_set)->hash;
        if (!$tag_exists) {    
            $c->response->redirect( $c->uri_for( "/topics/list/", { error_msg => "set tag $set_tag_name does not exist" } ) );
            return;
        }
                                         
    }
    
    $c->dbis->query("insert into word_cloud_topics " . 
                    ...
                    $query, 'words', $start_date, $end_date, 'pending', $c->uri_for ('/topics/'),
                    $source_tags_id, $set_tag_names, $creator);
