...
sub get_generator_from_xml 
{
    ...
    # TODO: This will fail on valid XML (comments, say)
    if ($_[0] =~ /generator>([^<]*)</) {
...
{
    my $dates = [];
    for ( my $date = Time::Local::timelocal(0, 0, 0, 1, 3, 108);
	  $date < time();
	  $date += 86400 ) 
    {
...
sub parse_feed 
{
    ...
    # try to reparse after munging the xml.
    # only do this when needed to avoid expensive regexes.
    if ($err)
    {
...
sub add_archive_feed_downloads
{
	...
	my $host =  lc( ( URI::Split::uri_split($url) )[1] );
        $db->query("insert into downloads "
		   . "(feeds_id, url, host, download_time, type, state,"
		   . " priority, sequence, extracted)"
                   . " values(?, ?, ?, now(), ?, ?, ?, ?, ?)",
                   $feed->{feeds_id}, $url, $host, 'feed', 'pending', 10, 1, 'f');
    }
}
