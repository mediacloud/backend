...
# strip the html from the text
sub _strip_html
{
    $_html_stripper ||= HTML::Strip->new;
    $_html_stripper->eof();

    return $_html_stripper->parse( $_[0] || '' );
}
...
sub get_text_from_download_text
{
    ...
    return join( "\n***\n\n" , _strip_html($story->{title}), _strip_html($story->{description}), @{$download_texts} );
}
...
"SELECT stm.tags_id FROM stories_tags_map stm, tags where stories_id = ? and stm.tags_id=tags.tags_id and tags.tag_sets_id = ?",
        $story->{stories_id}, $tag_set->{tag_sets_id}
    )->flat;
...
sub _add_module_tags
{
    ...
    $db->query(
        ...
        $tag_set->{tag_sets_id}, $story->{stories_id}
    );
    ...
    my $subtable_name = MediaWords::DBI::StoriesTagsMapMediaSubtables::get_or_create_sub_table_name_for_media_id($media_id);
    ...
    for my $tags_id (@tags_ids)
    {
        my $put_statement = join( "\t", $media_id, $story->{publish_date}, $story->{stories_id}, $tags_id, $tag_set->{tag_sets_id} ) . "\n";
...
# process story for vectors and add entries to story_sentences, story_sentence_words, and story_words tables
sub add_vectors
{
    my ( $db, $story ) = @_;
    
    # implmented as a server side function for performance.  see MediaWords::Pg::StoryVectors::update_story_sentence_words
    $db->query( "select update_story_sentence_words ('$story->{stories_id}')" );
}
...
sub get_db_module_tags
{
    ...
    return $db->query( "SELECT t.* FROM stories_tags_map stm, tags t, tag_sets ts " . 
                       "  where stm.stories_id = ? and stm.tags_id = t.tags_id " . 
                       "    and t.tag_sets_id = ts.tag_sets_id and ts.name = ?",
                       $story->{stories_id}, $module )->hashes;
}
