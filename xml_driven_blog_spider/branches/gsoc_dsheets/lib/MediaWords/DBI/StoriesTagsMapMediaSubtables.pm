...
sub sub_table_exists
{
    ...
    my @sub_table_names       = @{
        $db->query(
            "select tablename from pg_tables where schemaname = ? and tablename = ? ", $sub_table_schema_name,
...
sub create_sub_table_for_media_id
{
    ...
    $dbh->query_only_warn_on_error( 'CREATE TABLE ' . ' '
          . $stories_tags_media_sub_table . ' ( '
