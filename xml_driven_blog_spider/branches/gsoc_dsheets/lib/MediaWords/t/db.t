...
eval {
    MediaWords::Util::Config::set_config_file(__DIR__ . '/db_invalid.yml');
};

isnt($@, '', 'using nonexistant config files should croak');

eval {
    MediaWords::Util::Config::set_config_file(__DIR__ . '/db_empty.yml');
};
...
is($connect, 'dbi:Pg:dbname=mediacloud;host=localhost',
...
is($connect, 'dbi:Pg:dbname=mediacloudtest;host=localhost',
   'labeled connection string creation');
