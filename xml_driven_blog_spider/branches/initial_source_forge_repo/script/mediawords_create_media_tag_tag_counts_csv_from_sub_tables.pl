...
sub get_stories_map_with_tags
{
    ...
    my $stories_tags_map_rows =
      $dbh->query_only_warn_on_error( "select media_id, stories_id, tags_id  from "
          . $stories_tags_map_sub_table_name
          . " where publish_date > (now() - interval '90 days') order by stories_id" );

    $stories_tags_map_rows->bind( my ( $media_id, $stories_id, $tags_id ) );
...
sub get_tag_set
{
    ...
    #return 13;
    return get_tags_map()->{$tags_id}->{tag_sets_id} || die "Tag set not found by tag id $tags_id";
}
    ...
    foreach my $tag ( sort { $a <=> $b } keys %{$current_media_tag_hash_count} )
    {
        ...
        if ( $current_media_tag_hash_count->{$tag} )
        {
            ...
            foreach my $tag_tag (@top_ten_tag_tags)
            {
                ...
                {
                    my $tag_tag_name_full = get_tag_string( $tag_tag, $dbh );
                    my $tag_tag_name = $tag_tag_name_full;
                    $tag_tag_name =~ s/^the //;

                    if ( index( $media_name, $tag_tag_name ) != -1 )
                    {
                        print STDERR "Skipping $tag_tag_name ($tag_tag_name_full) on $media_name\n";
                        $tag_matches_source_name->{$tag_tag} = 1;

                        #print STDERR "Add $tag_tag id to hash\n";
                        #print ' $tag_matches_source_name->{$tag_tag} = ' . " $tag_matches_source_name->{$tag_tag} \n";
                        next;
                    }
                    else
                    {
                        $tag_matches_source_name->{$tag_tag} = 0;
                    }
                }
