...
sub main
{
    ...
    my Readonly $usage =
      'USAGE: ./mediawords_training_downloads_to_xml.pl --file=FILE_NAME ';
    ...
    my $downloads_rs  = $db->query("select distinct(downloads.*) from downloads natural join extractor_training_lines order by downloads_id");
