...
sub main {    
    ...
    eval {
        $db->query("create table duplicate_stories (stories_id int, feeds_id int)");
    };
    ...
    for (my $date = timelocal(0, 0, 0, 1, 3, 107); $date < time; $date += 86400) {
        ...
        my $story_groups = $db->query("select count(*) as num_stories, max(stories_id) as max_stories_id, title, media_id " . 
                                      "  from stories " . 
                                      "  where publish_date >= date '$date_ymd' " . 
                                      "    and publish_date < date '$date_ymd' + interval '1 day' " .
        ...
        for my $story_group (@{$story_groups})
        {
            ...
            $db->query( "insert into duplicate_stories (stories_id, feeds_id) " . 
            ...
            $db->query("insert into feeds_stories_map (stories_id, feeds_id) " .
                       "  select distinct ?::int, feeds_id from duplicate_stories where feeds_id is not null",
                       $story_group->{max_stories_id});
            
            $db->query("delete from stories where stories_id in (" . 
