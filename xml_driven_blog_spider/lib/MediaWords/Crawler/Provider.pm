...
sub _queue_download
{
    ...
    {
        $media_id = $self->_get_download_media_impl( $download->{feeds_id} );
    }
...
"update downloads set (state,error_message) = ('pending','') where state='error' and (type = 'spider_blog_home' or type = 'spider_posting' or type = 'spider_rss') and (error_message like 'r0%' or error_message like 'Download timed out by Fetcher:%') ;"
    );
}
