...
import mediawords.posts.crimson_hexagon_twitter
import mediawords.posts.archive_org_twitter
import mediawords.posts.csv_generic
...
def _insert_post_urls(db: DatabaseHandler, topic_post: dict, urls: list) -> list:
...
def _get_post_urls(post: dict) -> list:
    ...
    if 'tweet' in post:
        return mediawords.util.twitter.get_tweet_urls(post['tweet'])
...
def _store_post_and_urls(db: DatabaseHandler, topic_post_day: dict, post: dict) -> None:
    ...
    data_json = mediawords.util.parse_json.encode_json(post)
...
def regenerate_post_urls(db: dict, topic: dict) -> None:
...
def fetch_topic_posts(db: DatabaseHandler, topics_id: int, max_posts_per_day: typing.Optional[int] = None) -> None:
    ...
    db - database handle
    topics_id - topic id
    max_posts_per_day - max posts to fetch each day
