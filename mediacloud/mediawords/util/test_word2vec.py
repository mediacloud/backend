...
from mediawords.util.word2vec.model_stores import TopicDatabaseModelStore, SnapshotDatabaseModelStore
from mediawords.util.word2vec.sentence_iterators import TopicSentenceIterator, SnapshotSentenceIterator
...
class TestWord2vec(TestDatabaseWithSchemaTestCase):
    ...
    def test_topic_sentence_iterator(self):
        # Nonexistent topic
        with pytest.raises(McWord2vecException):
            TopicSentenceIterator(db=self.db(), topics_id=123456)
    ...
    def test_train_word2vec_model_topic(self):

        temp_directory = tempfile.mkdtemp()

        fixtures = [
            (
                TopicSentenceIterator(db=self.db(), topics_id=self.topics_id),
                TopicDatabaseModelStore(db=self.db(), topics_id=self.topics_id),
            ),
            (
                SnapshotSentenceIterator(db=self.db(), snapshots_id=self.snapshots_id),
                SnapshotDatabaseModelStore(db=self.db(), snapshots_id=self.snapshots_id),
            ),
        ]

        for sentence_iterator, model_store in fixtures:
