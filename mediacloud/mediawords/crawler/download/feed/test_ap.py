...
class TestAPFetcher(TestCase):
    ...
    def mock_story_data(self,feed_data:str) -> None:
        ...
        for item in fixture_data_stories:
            ...
            nitf_mock_body = open(self.fixture_data_dir + "test_ap_fixture_{guid}.nitf".format(guid=guid),"r").read().rstrip()

            # Register mock content responses
            httpretty.register_uri(httpretty.GET, mock_content_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = content_mock_body)

            # Register mock nitf responses
            httpretty.register_uri(httpretty.GET, mock_nitf_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = nitf_mock_body)
    ...
    def setUp(self):
        ...
        fixture_search_data = open(self.fixture_data_dir + "test_ap_fixture_search_data","r").read()
        ...
        self.fixture_content_data = json.loads(open(self.fixture_data_dir + "test_ap_fixture_content_data","r").read())
        ...
        httpretty.register_uri(httpretty.GET, self.MOCK_FEED_ENDPOINT_URL, adding_headers=self.MOCK_RESPONSE_HEADERS, body = self.fixture_feed_data)
        httpretty.enable()
...
>>>>>>> pushshift/master
    ...
    def test_get_new_stories(self) -> None:
        ...
        # Register Feed mock endpoint
        httpretty.register_uri(httpretty.GET, self.MOCK_FEED_ENDPOINT_URL, adding_headers = self.MOCK_RESPONSE_HEADERS, body = json.dumps(fixture_feed_data))

        # Set up Mock Search Feed data and two fake story contents since all feed endpoint data is younger than max_lookback and search method will be invoked
        ...
        mocked_nitf_content_data = open(self.fixture_data_dir + "test_ap_fixture.nitf".format(guid=guid),"r").read().rstrip()
        ...
        httpretty.register_uri(httpretty.GET, mock_content_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = json.dumps(mocked_content_data['fake_id_1']))
        nitf_mock_body = open(self.fixture_data_dir + "test_ap_fixture_{guid}.nitf".format(guid=guid),"r").read().rstrip()
        mock_nitf_url = "https://api.ap.org/media/v/content/{guid}.{version}/download".format(guid='fake_id_1',version=0)
        httpretty.register_uri(httpretty.GET, mock_nitf_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = nitf_mock_body)
        ...
        httpretty.register_uri(httpretty.GET, mock_content_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = json.dumps(mocked_content_data['fake_id_2']))
        nitf_mock_body = open(self.fixture_data_dir + "test_ap_fixture_{guid}.nitf".format(guid=guid),"r").read().rstrip()
        mock_nitf_url = "https://api.ap.org/media/v/content/{guid}.{version}/download".format(guid='fake_id_2',version=0)
        httpretty.register_uri(httpretty.GET, mock_nitf_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = nitf_mock_body)
        ...
        httpretty.register_uri(httpretty.GET, mock_content_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = json.dumps(mocked_content_data['fake_id_3']))
        nitf_mock_body = open(self.fixture_data_dir + "test_ap_fixture_{guid}.nitf".format(guid=guid),"r").read().rstrip()
        mock_nitf_url = "https://api.ap.org/media/v/content/{guid}.{version}/download".format(guid='fake_id_3',version=0)
        httpretty.register_uri(httpretty.GET, mock_nitf_url, adding_headers=self.MOCK_RESPONSE_HEADERS, body = nitf_mock_body)

        httpretty.register_uri(httpretty.GET, MOCK_SEARCH_ENDPOINT_URL, adding_headers = self.MOCK_RESPONSE_HEADERS, body = json.dumps(fixture_search_data))

        # There should only be 5 stories. The get_new_stories() method will stop processing stories once it reaches one beyond the max_lookback and returns
        # up to one story passed the max_lookback parameter
