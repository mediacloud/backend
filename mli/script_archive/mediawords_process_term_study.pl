...
sub main
{
    ...
    my @stories_rs = $db->resultset( 'Stories' )->search(
        {
            'feeds_stories_maps.feeds_id' => $feeds_id,
            -and                          => [
                publish_date => { '>=', $time_formatted },
    ...
    foreach my $story ( @stories_rs )
    {
        ...
        {
            my $found_story_texts_rs =
              $db->resultset( 'StoryTexts' )
              ->search( { 'story_texts_id' => $story->get_column( 'story_texts_id' ) }, { join => 'story' } );
        ...
        my @downloads_rs =
          ...
          ->search( { 'stories_id' => $story->get_column( 'stories_id' ) }, { order_by => 'parent asc' } );
...
# NOTE:  media_id=2 means Washington Post, media_id=26 means Sacramento Bee, media_id=115 means Daily Kos, media_id=128 means Red State
                ...
                if ( $story->get_column( 'media_id' ) == 2 )
                {
                    ...
                    {
                        $extracted_content =
                          MediaWords::Crawler::Parser::String->get_story_text( "", $content, "article_body",
                    ...
                    {
                        $extracted_content =
                          MediaWords::Crawler::Parser::String->get_story_text( "", $content, "content", "commentText" );
                        $extracted_content =
                          MediaWords::Crawler::Parser::String->get_story_text( "", $extracted_content, "content", "posted" );
                    }
                    if ( $extracted_content eq '' )
                    {
                        $extracted_content =
                          MediaWords::Crawler::Parser::String->get_story_text( "", $content, "storyText", "" );
                    }
                }
                elsif ( $story->get_column( 'media_id' ) == 26 )
                {
                    $extracted_content =
                      MediaWords::Crawler::Parser::String->get_story_text( "", $content, "storyBody", "thickbox" );
                    $extracted_content =
                      MediaWords::Crawler::Parser::String->get_story_text( "", $extracted_content, "storyBody", "simg" );
                }
                ...
                {
                    $extracted_content =
                      MediaWords::Crawler::Parser::String->get_story_text( "", $content, "post-body", "" );
                    if ( $extracted_content eq '' )
                    {
                        $extracted_content =
                          MediaWords::Crawler::Parser::String->get_story_text( "", $content, "secondarystory", "h4" );
                    }
                }
                else
                {
        ...
        if ( $encoded_text ne "" )
        {
         ...
         # create a new StoryTexts object associated with the story and with the aggregate_story_text as its story_text field
            my $new_story_texts = $db->resultset( 'StoryTexts' )->create( { story_text => $encoded_text, } );
