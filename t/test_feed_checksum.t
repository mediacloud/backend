...
sub test_feed_checksums
{
    ...
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_a ), 0, "empty checksum" );
    
    # next check with same stories should be a match
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_a ), 1, "match 1" );

    # and another match
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_a ), 1, "match 2" );
    
    # and now try with different set of stories
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_b ), 0, "fail 1" );
    
    # and now with the same b stories
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_b ), 1, "match 3" );
    
    # and now add one story
    push( @{ $stories_b }, { url => 'http://foo.bar.com' } );
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_b ), 0, "fail 2" );
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_b ), 1, "match 4" );
    ...
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_a ), 0, "fail 4" );
    is ( MediaWords::Crawler::FeedHandler::stories_checksum_matches_feed( $db, $feed->{ feeds_id }, $stories_a ), 1, "match 5" );
}
