...
sub test_use_pager
{
    ...
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 1, "use_pager true" );

    $medium = $db->query( "update media set use_pager = null where media_id = ? returning *", $medium->{ media_id } )->hash;
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 1, "null use_pager" );

    $medium = $db->query( "update media set use_pager = 'f' where media_id = ? returning *", $medium->{ media_id } )->hash;
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 0, "use_pager false" );

    $medium = $db->query( "update media set use_pager = null where media_id = ? returning *", $medium->{ media_id } )->hash;
    MediaWords::Crawler::Handler::_set_use_pager( $db, $medium, 'http://foo.bar' );
    $medium = $db->find_by_id( 'media', $medium->{ media_id } );
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 1, "set_use_pager use_pager true" );

    $medium = $db->query( <<END, $medium->{ media_id } )->hash;
update media set use_pager = null, unpaged_stories = 99 where media_id = ? returning *
END
    MediaWords::Crawler::Handler::_set_use_pager( $db, $medium, undef );
    $medium = $db->find_by_id( 'media', $medium->{ media_id } );
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 1, "100th unpaged story: use_pager true" );

    MediaWords::Crawler::Handler::_set_use_pager( $db, $medium, undef );
    $medium = $db->find_by_id( 'media', $medium->{ media_id } );
    is( MediaWords::Crawler::Handler::_use_pager( $medium ), 0, "100th unpaged story: use_pager false" );
}
