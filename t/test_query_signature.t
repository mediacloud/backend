...
sub create_query_parts
{
    ...
    for my $i ( 1 .. 4 )
    {
        my $media_set = $db->create( 'media_sets', { name => "qs test $i", set_type => 'collection', tags_id => $tag->{ tags_id } } );
    ...
    for my $i ( 1 .. 4 )
    {
        my $dashboard = $db->create( 'dashboards', { name => "qs test $i", start_date => '2010-01-01', end_date => '2020-01-01' } );
              
        for my $j ( 1 .. 4 ) 
        {
            my $dashboard_topic = $db->create( 'dashboard_topics', 
                { name => "qs test $i", 
                  query => "foo", 
        ...
        for my $j ( 1 .. $i )
        {
            $db->create( 'dashboard_media_sets', 
                { dashboards_id => $dashboard->{ dashboards_id }, 
...
sub test_query_signatures
{
    ...
    map { $_->{ create_query } = MediaWords::DBI::Queries::find_or_create_query_by_params( $db, $_->{ params } ) } @{ $tests };
