...
sub add_test_feed
{
    ...
    my $test_medium = $db->query( "insert into media (name, url, moderated, feeds_added) values (?, ?, ?, ?) returning *", 
                                  '_ Crawler Test', $url_to_crawl, 0, 0 )->hash;
    my $feed = $db->query( "insert into feeds (media_id, name, url) values (?, ?, ?) returning *",
                           $test_medium->{ media_id }, '_ Crawler Test',
                           "$url_to_crawl" . "gv/test.rss" )->hash;
...
sub get_expanded_stories
{
    ...
    for my $story ( @{ $stories } )
    {
        ...
        $story->{ story_sentence_words } = $db->query( "select * from story_sentence_words where stories_id = ?", $story->{ stories_id } )->hashes;
    }
