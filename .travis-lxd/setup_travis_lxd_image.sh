...
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- mkdir -p $MC_LXD_MEDIACLOUD_ROOT/ansible/
sudo $LXC_BIN file push --recursive $PWD/../ansible/* $MC_LXD_CONTAINER/$MC_LXD_MEDIACLOUD_ROOT/ansible/
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- chown -R $MC_LXD_USER:$MC_LXD_USER $MC_LXD_MEDIACLOUD_ROOT

echo "Provisioning container with Ansible..."
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- sudo -H -u $MC_LXD_USER /bin/bash -c \
    "cd $MC_LXD_MEDIACLOUD_ROOT/ansible/;
    ansible-playbook --inventory='localhost,' --connection=local -vvv travis.yml"

echo "Cleaning up container..."
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- rm -rf $MC_LXD_MEDIACLOUD_ROOT
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- rm -rf $MC_LXD_USER_HOME/.cache/
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- rm -rf $MC_LXD_USER_HOME/.cpanm/
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- rm -rf /root/.cache/
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- apt-get -y clean
sudo $LXC_BIN exec $MC_LXD_CONTAINER -- apt-get -y autoremove

echo "Stopping container..."
sudo $LXC_BIN stop mediacloud-travis

MC_LXD_ALIAS="$MC_LXD_CONTAINER-`date +%Y%m%d`"

echo "Publishing image as '$MC_LXD_ALIAS'..."
sudo $LXC_BIN publish $MC_LXD_CONTAINER --alias $MC_LXD_ALIAS --public --verbose

echo "Exporting image '$MC_LXD_ALIAS'..."
sudo $LXC_BIN image export $MC_LXD_ALIAS $MC_LXD_ALIAS
