...
-- versions 4688 and 4689.
--
-- If you are running Media Cloud with a database that was set up with a schema version
-- 4688, and you would like to upgrade both the Media Cloud and the
-- database to be at version 4689, import this SQL file:
--
--     psql mediacloud < mediawords-4688-4689.sql
--
...
DROP FUNCTION stories_partition_chunk_size();
DROP FUNCTION stories_partition_name(base_table_name TEXT, stories_id INT);
DROP FUNCTION stories_create_partitions(base_table_name TEXT);


CREATE OR REPLACE FUNCTION partition_by_stories_id_chunk_size() RETURNS BIGINT AS $$
BEGIN
    RETURN 100 * 1000 * 1000;   -- 100m stories in each partition
END; $$
LANGUAGE plpgsql IMMUTABLE;

CREATE OR REPLACE FUNCTION partition_by_stories_id_partition_name(base_table_name TEXT, stories_id INT) RETURNS TEXT AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION story_sentences_create_partitions() RETURNS VOID AS
$$
DECLARE
    created_partitions TEXT[];
    partition TEXT;
