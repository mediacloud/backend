...
sub main 
{
    ...
    my $drop_days = $db->query( 
        ...
        "  order by cur.publish_day, ms.media_sets_id"
    )->hashes;
        
    for my $day ( @{ $drop_days } )
    {
        print "90% drop: $day->{ name } / $day->{ cur_day } / $day->{ cur_count } / $day->{ prev_day } / $day->{ prev_count }\n";
    }
    
    my $topic_days = $db->query(
