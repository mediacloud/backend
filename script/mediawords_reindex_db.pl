...
{
    my $warning_message =
"Warning this script will take a very long time. Are you sure you wish to continue?";
    ...
    my $index_hashes = $db->query("select p.relname from pg_class p, pg_stat_user_indexes pg where pg.indexrelname = p.relname order by p.relpages desc;  ")->hashes;

    my @indexes = map { $_->{relname} } @$index_hashes;

    #say STDERR Dumper([@indexes]);

    my $non_reindexed_prefixes = [ qw ( story_sentence_words_ downloads_ stories_ daily_words_ ) ];
