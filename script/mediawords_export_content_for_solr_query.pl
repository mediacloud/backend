...
sub main
{
    ...
    die( "usage: $0 -q <solr query> -d <output dir> [-c text|sentences|raw ]" ) unless ( $query && $dir );
    ...
    my $stories_ids = MediaWords::Solr::search_for_stories_ids( $db, { q => $query, rows => $MAX_ROWS } );
