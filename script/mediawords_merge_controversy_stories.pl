...
sub merge_dup_story
{
    ...
    print STDERR "merging $story->{ title } [ $story->{ stories_id } ] <- $dup_story->{ title } [ $dup_story->{ stories_id } ]\n";
        
    $db->query( "update controversy_links set ref_stories_id = ? where ref_stories_id = ?",
        $story->{ stories_id }, $dup_story->{ stories_id } );
...
sub merge_duplicate_stories
{
    ...
    for my $story ( @{ $stories } )
    {
        ...
        my $dup_stories = $db->query( 
            ...
            $story->{ media_id }, $story->{ title }, $story->{ stories_id } )->hashes;
        for my $dup_story ( @{ $dup_stories } )
        {
            ...
            if ( ( $dup_story_epoch < ( $story_epoch - ( 7 * 86400 ) ) ) || ( $dup_story > ( $story_epoch + ( 7 * 86400 ) ) ) )
            {
