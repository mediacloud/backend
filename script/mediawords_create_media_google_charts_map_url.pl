...
sub get_country_code_to_tag_count
{
    ...
    for my $tag_count_row ( @{$tag_counts} )
    {
        ...
        $country_code_count->{$country_code} = max( $country_code_count->{$country_code}, $tag_count_row->{tag_count} );
    }
...
sub get_diff_tag_count_map_url
{
    ...
    my $data = join( ',',
        map { int( ( ( $country_code_count->{$_} / $max_tag_count ) * 100 ) / 2 ) + 50 } sort keys %{$country_code_count} );
    my $countrycodes = join( '', sort keys %{$country_code_count} );
    my $esc_title = uri_escape($title);
...
#cat country_code_list.txt  | sed -e 's/\w*:\w*:\w*:[^:]*//' | uniq |  tr ':' '\n' | grep -v '^$' | sort | perl -e 'while (<>) { chomp; print "\"$_\",\n"; } ' | tr \" \'
            ...
            my @media_tag_counts = (
                $db->query(
                    "select c.*, t.tag from media_tag_counts c, tags t "
                      . "where c.tags_id = t.tags_id and c.media_id = "
                      . ( $media_id + 0 )
                      . " and t.tag_sets_id = "
                      . ( $tag_sets_id + 0 )
                      . " and  t.tag in (??) "
    ...
    $db->query( "CREATE TABLE $temp_table_name ( "
          ...
          . ", chart_type_is_log  BOOLEAN NOT NULL"
          . ", chart_url text) " )
      or die $db->error;
    ...
    foreach my $row_to_insert (@rows_to_insert_2)
    {
        ...
        $db->query(
            ...
            $row_to_insert->{media_id},          $row_to_insert->{tag_sets_id},
            $row_to_insert->{chart_type_is_log}, $row_to_insert->{chart_url},
        );
    }
