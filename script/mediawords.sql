...
CREATE OR REPLACE FUNCTION create_schema_enum()
RETURNS BOOLEAN AS $$
    ...
    COMMENT ON FUNCTION enum.enum_add (enum_name character varying, enum_elem character varying)
        IS 'Inserts a new ENUM element wthout re-creating the whole type.';
    COMMENT ON FUNCTION enum.enum_del (enum_name character varying, enum_elem character varying)
        IS 'Removes the ENUM element "on the fly". Check references to the ENUM element in database''s 
        tables before the deletion and throws an exception if the element cannot be deleted.';
...
DROP FUNCTION IF EXISTS create_schema_enum();
