...
"SELECT table_name FROM information_schema.tables WHERE table_type <> 'VIEW' and table_schema = 'public' and not ( table_name like 'sopa_date_counts_dump_00%' ) and not ( table_name like 'sopa_links_dump_00%'  ) and not ( table_name like 'sopa_media_dump%' ) and not ( table_name like 'sopa_%' ) order by table_name asc";
    ...
    foreach my $table ( @tables )
    {
        ...
        my $query =
          "select * from $table where $table" . "_id in (select floor(random() * (max_id - min_id + 1))::integer + min_id " .
          " from generate_series(1,500), (select max($table" . "_id) as max_id, min($table" .
          "_id) as min_id from $table) s1 " . "        limit 500)  order by random() limit 200; ";
