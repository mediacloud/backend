...
sub gen_term_frequency
{
    ...
    $db->query( "CREATE TEMPORARY TABLE temp_term_counts (
                    term        VARCHAR(256) NOT NULL
                 ) WITH (OIDS=FALSE)" );

    # Sentence count (to give a sense of progress)
    my $sentence_count = $db->query( "SELECT reltuples
                                      FROM pg_class
                                      WHERE relname = 'story_sentences'")->hash()->{ reltuples };
    ...
    # Print term count
    my $term_count_rs = $db->query( "SELECT
                                    term,
