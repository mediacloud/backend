...
function displayNode(_nodeIndex, _recentre) {
    ...
    if (_nodeIndex != -1) {
        ...
        for (var i in _d.attributes) {
            _str += '<li><b>' + i + '</b> : ' + replaceURLWithHyperlinks( _d.attributes[i] ) + '</li>';
        }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var _g = $(data).find("graph"),
                ...
                _attributes = _g.children().filter("attributes").children();
            GexfJS.graph = {
            ...
            $(_attributes).each( function() {
                var _n = $(this);
                _attributeNames[ _n.attr("id") ] = _n.attr("title");
            });
            ...
            $(_nodes).each( function() {
                ...
                $(_attr).each(function() {
                    var _a = $(this),
                        _for = _a.attr("for");                    
                    _d.attributes[ _for ? _attributeNames[ _for ] : 'attribute_' + _a.attr("id") ] = _a.attr("value");
                });
