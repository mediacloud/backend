...
sub main
{
    ...
    for my $topic ( @{ $topics } )
    {
        ...
        MediaWords::StoryVectors::update_aggregate_words( $db, '2010-04-01', $topic->{ start_date },
            0, $topic->{ dashboard_topics_id } );
    }
    ...
    {
        print STDERR "updating: $topic->{ name }\n";
        MediaWords::StoryVectors::update_aggregate_words( $db, $topic->{ end_date },
            $yesterday, 0, $topic->{ dashboard_topics_id } );
    }
