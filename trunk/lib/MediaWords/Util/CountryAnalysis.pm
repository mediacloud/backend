...
my $_comparison_functions =
    [
        [ 'cloud', \&get_comparative_word_cloud_url ],
    ];
...
sub fetch_media_set
{
    ...
    {
        $fetched_media_set = $db->query( 'select * from media_sets where media_sets_id = ?', $media_set->{ media_sets_id } )->hash ||
...
sub fetch_media_sets
{
    ...
    {
        $media_set_group->{ media_sets } = [ map { get_media_set( $db, $_ ) } @{ $media_set_group->{ media_set_nicknames } } ];
    }   
}
...
sub find_or_create_polar_map
{
    ...
    my $cluster_map = $db->query( 
        ...
        $polar_query->{ queries_id }, $cluster_run->{ media_cluster_runs_id } )->hash;
    ...
    {
        $cluster_map = MediaWords::Cluster::Map::generate_cluster_map( $db, $cluster_run, 'polar', [ $polar_query ], 0, 'graphviz-neato' );
    }
...
sub get_topic_results
{
    ...
    {
        execute_functions( $_pole_functions, $results, $db, $topic, $pole_media_set->{ nickname }, $pole_media_set->{ nickname } );
    }
    
    my $media_set_groups = get_all_media_sets_as_groups( );
    for my $media_set_group ( @{ $media_set_groups } )
    {
        execute_functions( $_media_set_functions, $results, $db, $topic, $media_set_group->{ nickname }, $media_set_group->{ media_sets } );
        
        for my $pole_media_set ( @{ $_pole_media_sets } )
        {
            my $label = "$media_set_group->{ nickname }_$pole_media_set->{ nickname }_pole";
            execute_functions( $_media_set_pole_functions, $results, $db, $topic, 
                $label, $media_set_group->{ media_sets }, $pole_media_set->{ nickname } );
        }
    }
    ...
    {
        execute_functions( $_comparison_functions, $results, $db, $topic, $media_set_pair->{ nickname }, 
            $media_set_pair->{ a }, $media_set_pair->{ b } );
    }
