...
sub get_color
{
    ...
    my $all_dashboard_media_sets = $db->query(
        ...
        "  where ms.media_sets_id = dms.media_sets_id and ms.set_type = 'collection' " .
        "    and dms.dashboards_id = ?", 
        $dashboard_media_set->{ dashboards_id } )->hashes;
    ...
    for my $dms ( @{ $all_dashboard_media_sets } )
    {
        if ( $dms->{ color } )
        {
            ...
            $return_color = $dms->{ color } if ( $dms->{ dashboard_media_sets_id } = $dashboard_media_set->{ dashboard_media_sets_id } );
        }
        else
        {
    ...
    for my $dms ( @{ $colorless_dashboard_media_sets } )
    {
        ...
        $db->query ( "update dashboard_media_sets set color = ? where dashboard_media_sets_id = ?",
            $dms->{ color }, $dms->{ dashboard_media_sets_id } );
