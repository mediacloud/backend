...
sub _get_description
{
    ...
    if ( @{ $media_sets } > 2 )
    {
        $description = "in one of " . @{ $media_sets } . " media sources including " . 
    ...
    {
        $description .= " mentioning one of " . @{ $dashboard_topics } . " topics including " . 
...
sub _create_query
{
    ...
    for my $table ( 'dashboard_topics', 'media_sets' )
    {
        ...
        {
            $db->query( 
                "insert into queries_${table}_map ( queries_id, ${table}_id ) values ( ?, ? ) ",
                $queries_id, $id );
        }
    }
...
sub _add_mapped_table_fields
{
    ...
    for my $table ( 'dashboard_topics', 'media_sets' ) 
    {
        ...
        if ( !@{ $query->{ $ids_field } } )
        {
            ...
            $query->{ $table } = [ $db->query( "select * from $table where ${table}_id in ( $ids_list ) order by ${table}_id" )->hashes ];
        }
    }
}
...
sub find_query_by_params
{
    ...
    my ( $queries_id ) = $db->query( "select queries_id from queries where md5( description ) = md5( ? )", $description )->flat;
    ...
    for my $table ( 'media_sets', 'dashboard_topics' )
    {
        $query->{ $table } = 
            [ $db->query( 
                ...
                "  where m.${table}_id = qm.${table}_id and qm.queries_id = $queries_id order by m.${table}_id" )->hashes ];
...
sub _get_stories_from_sentences
{
    ...
    my $stories = $db->query( 
        "select * from stories where stories_id in ( $stories_ids_list ) " .
        "  order by publish_date" )->hashes;
...
sub get_number_of_media_sources
{
    ...
    my ( $num_media_sources ) = $db->query( 
        "select count(distinct media_id) from media_sets_media_map " . 
        "  where media_sets_id in ( $media_set_in_list )" )->flat;
...
sub find_or_create_media_sub_query
{
    ...
    my $media_sets_ids = [ $db->query( 
        "select media_sets_id from media_sets " . 
        "  where media_id in ( $media_ids_list ) " )->flat ];
...
sub get_media_with_sub_queries
{
    ...
    my $media = $db->query( 
        ...
        "    and qmsm.queries_id = $query->{ queries_id } and ms.media_id = m.media_id " .
        "  order by m.name" )->hashes;
        
    for my $medium ( @{ $media } )
    {
        $medium->{ query } = MediaWords::DBI::Queries::find_or_create_query_by_params( $db, {
            start_date => $query->{ start_date },
            end_date => $query->{ end_date },
            dashboard_topics_ids => $query->{ dashboard_topics_ids },
