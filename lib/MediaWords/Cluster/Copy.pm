...
sub get_clusters
{
    ...
    my $source_clusters =
      $db->query( "select * from media_clusters where media_cluster_runs_id = ? order by media_clusters_id asc ",
    ...
    for my $source_cluster ( @{ $source_clusters } )
    {
        ...
        $dest_cluster->{ media_ids } = [
            $db->query( "select media_id from media_clusters_media_map where media_clusters_id = ?",
