...
sub _get_download_from_less_trained_media_source
{
    ...
    {
        $downloads = $c->dbis->query("SELECT downloads.* from downloads, stories where type='content' and state='success' and stories.stories_id=downloads.stories_id and stories.media_id in (select media_id from (select * from media_adjusted_extractor_training_downloads_count order by count limit 20) as media_20 order by count desc limit 15) limit 100")->hashes;
    }
