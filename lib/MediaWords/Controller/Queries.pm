...
sub get_query_form
{
    ...
    my $dashboard_topics = $c->dbis->query(
        "select dt.*, d.name as dashboard_name from dashboard_topics dt, dashboards d " . 
...
sub sentences : Local
{
    ...
    my $queries = [ map { MediaWords::DBI::Queries::find_query_by_id( $c->dbis, $_ ) || die( "no query $_" ) } @{ $queries_ids } ];
    ...
    my $num_sentences = 0;
    map { map { $num_sentences++ } @{ $_->{ sentences } } } @{ $stories };
