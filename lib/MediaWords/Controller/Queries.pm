...
sub queue_story_search : Local
{
    ...
    if ( !$form->submitted_and_valid() )
    {
        my $searches = $c->dbis->query( 
            "select * from query_story_searches where queries_id = ? order by query_story_searches_id asc", $queries_id )->hashes;
    ...
    my $existing_search = $c->dbis->query( 
        "select * from query_story_searches where queries_id = ? and pattern = ?",
        $queries_id, $pattern )->hash;
        
    if ( $existing_search )
    {
        $c->response->redirect( 
            $c->uri_for( "/queries/queue_story_search/$query->{ queries_id }", { status_msg => 'Story search already exists.' } ) );
        return;
    }
        
    $c->dbis->create( 'query_story_searches',
        {
    ...
    $c->response->redirect( $c->uri_for( "/queries/queue_story_search/$query->{ queries_id }", { status_msg => 'Story search queued.' } ) );
}
