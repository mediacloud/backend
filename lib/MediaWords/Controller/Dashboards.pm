...
sub list_topics : Local
{
    ...
    my $dashboard_topics = $c->dbis->query( "select * from dashboard_topics where dashboards_id = ? order by dashboard_topics_id",
...
sub create_topic : Local
{
    ...
    $c->dbis->query( "insert into dashboard_topics ( name, query, start_date, end_date, dashboards_id ) " .
        " values( substring( ?, 0, 256 ), substring( ?, 0, 1024) , " . 
        "   date_trunc( 'week', ?::date ), date_trunc( 'week', ?::date ) + interval '1 week', ? ) ",
        $c->req->param( 'name' ), $stemmed_query, $c->req->param( 'start_date' ), $c->req->param( 'end_date' ),
        $dashboards_id );
