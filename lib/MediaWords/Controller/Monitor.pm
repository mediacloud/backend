...
"select date_trunc('day', download_time) as day, count(*) from downloads where date_trunc('day', download_time) >= ? and  date_trunc('day', download_time) <= ? group by date_trunc('day', download_time);", $start_date, $end_date
    )->hashes();
