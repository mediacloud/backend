...
sub get_media
{
    ...
    for my $medium ( @{ $media } )
    {
        ...
        $medium->{ last_week_stem_counts } = $db->query(
            "select publish_week, stem, term, stem_count from top_500_weekly_words " .
            "  where publish_week in " . 
            ...
            $medium->{ media_id }, $medium->{ media_id } )->hashes;
    }
}
...
END
    ...
    {
        $story->{ feed_urls } = $db->query( 
            "select f.url from feeds f, feeds_stories_map fsm where f.feeds_id = fsm.feeds_id and stories_id = ?", 
