...
sub _add_nested_data
{
    ...
    {
        my $media_sets = $db->query("select ms.media_sets_id, ms.name from media_sets ms, dashboard_media_sets dms where  " .
	...
	{
	    my $media = $db->query("SELECT m.name, m.url, m.media_id from media m , media_sets_media_map msmm" .
				   " WHERE m.media_id = msmm.media_id AND msmm.media_sets_id = ? ORDER BY m.media_id ",
				   $media_set->{ media_sets_id } )->hashes();
