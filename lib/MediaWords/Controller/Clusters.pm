...
sub list : Local
{
    ...
    my $cluster_runs = $c->dbis->query( 
        "select mcr.*, ms.name as media_set_name from media_cluster_runs mcr, media_sets ms " .
        "  where mcr.media_sets_id = ms.media_sets_id " .
        "  order by mcr.media_cluster_runs_id" )->hashes;
...
sub create : Local
{
    ...
    my $cluster_run = $c->dbis->create_from_request( 'media_cluster_runs', $c->request, 
...
sub view : Local
{
    ...
    my $media_clusters = $c->dbis->query( "select * from media_clusters where media_cluster_runs_id = ?", $cluster_runs_id )->hashes;
