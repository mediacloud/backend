...
sub query_term_in_media
{
    ...
    if ( @{$stems} == 1 )
    {
        ...
        if( @{ $params->{'media_type'} } > 0 )
        {
            ...
            push @{$conditions}, " total_daily_media_words.media_id in (select media_id from media_tags_map where tags_id = ?) ";
        ...
        else
        {
            ...
            if ( $params->{'from'} ne "" && defined $params->{'to'} ne "" )
            {
                $slice_counts->{ $stems->[0] } =
                $c->dbis->query( $query, $from, $stems->[0], $from, $to )->hashes;
            }
            else
            {
                $slice_counts->{ $stems->[0] } =
                $c->dbis->query( $query, $num_days, $stems->[0], $from, $to )->hashes;
            }
    ...
    else
    {
        ...
        if( @{ $params->{'media_type'} } > 0 )
        {
            $media = $c->dbis->query( " select  media_id from media_tags_map where tags_id in ( " . join( ',', map( '?', @{ $params->{'media_type'} } ) ) . " ) " , @{ $params->{'media_type'} } )->flat;
        }
        elsif( @{ $params->{'media'} } > 0 )
        {
            $media = $params->{'media'};
        ...
        if( defined $media && @{ $media } > 0 )
        {
            ...
            for ( my $i = 0 ; $i < @{$stems} ; $i++ )
            {
                ...
                if ( $params->{'from'} ne "" && defined $params->{'to'} ne "" )
                {
                    ...
                    $c->dbis->query( $query, $from, $stems->[$i], $from, $to )->hashes;
                }
                else
                {
                    $slice_counts->{ $stems->[$i] } =
