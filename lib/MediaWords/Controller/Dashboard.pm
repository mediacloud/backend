...
sub list : Local
{
    ...
    my $media = $c->dbis->query( 
        ...
        "  order by m.name", $dashboard->{ dashboards_id } )->hashes;
    ...
    my $collection_media_sets = $c->dbis->query( 
        ...
        "  order by ms.media_sets_id", $dashboard->{ dashboards_id } )->hashes;
        
    my $dashboard_topics = $c->dbis->query(
        "select * from dashboard_topics where dashboards_id = ?", $dashboard->{ dashboards_id } )->hashes;
...
sub get_media_set_clusters
{
    ...
    if ( $type eq 'medium' ) 
    {
        ...
        $clusters = $c->dbis->query( 
            ...
            "    dms.media_cluster_runs_id = mc.media_cluster_runs_id and " . 
            "    dms.dashboards_id = ?", 
            $media_set->{ media_id }, $dashboard->{ dashboards_id } )->hashes;
    }
    ...
    {        
        ...
        $clusters = $c->dbis->query( 
            ...
            "    dms.dashboards_id = ?",
            $media_set->{ media_sets_id }, $dashboard->{ dashboards_id } )->hashes;
    }
    elsif ( $type eq 'cluster' )
    {
        # for a cluster, get all of the other clusters in the same clustering run
        $clusters = $c->dbis->query( "select a.* from media_clusters a, media_clusters b " . 
                                     "  where a.media_cluster_runs_id = b.media_cluster_runs_id " .
                                     "    and b.media_clusters_id = ?",
                                     $media_set->{ media_clusters_id } )->hashes;
    }
    else {
    ...
    for my $mc ( @{ $clusters } )
    {
        $mc->{ media } = $c->dbis->query( "select m.* from media m, media_clusters_media_map mcmm " .
                                          "  where m.media_id = mcmm.media_id and mcmm.media_clusters_id = ?",
                                          $mc->{ media_clusters_id } )->hashes;

        $mc->{ internal_features } = $c->dbis->query( "select * from media_cluster_words " . 
                                                      "  where media_clusters_id = ? and internal = 't' " .
                                                      "  order by weight desc",
                                                      $mc->{ media_clusters_id } )->hashes;

        $mc->{ external_features } = $c->dbis->query( "select * from media_cluster_words " . 
                                                      "  where media_clusters_id = ? and internal = 'f' " .
                                                      "  order by weight desc",
                                                      $mc->{ media_clusters_id } )->hashes;
                                                      
        $mc->{ features } = [ @{ $mc->{ external_features } }, @{ $mc->{ internal_features } } ];
        
        $mc->{ media_set } = $c->dbis->query( "select * from media_sets where media_clusters_id = ?", 
                                              $mc->{ media_clusters_id } )->hash;
    }
...
sub get_media_set_from_params
{
    ...
    {
        return $c->dbis->query( "select ms.* from media_sets ms, media m " . 
                                "  where ms.media_id = m.media_id and m.name = ?", $medium_name )->hash 
...
sub compare_media_set_words
{
    ...
    my ( $max_stem_count_a ) = $db->query( "select max( stem_count ) " .
                                           ...
                                           $media_set_a->{ media_sets_id }, $date )->flat;                                           
    my ( $max_stem_count_b ) = $db->query( "select max( stem_count ) " .
                                           "  from ( select stem_count from top_500_weekly_words_normalized " .
                                           "      where media_sets_id = ? and publish_week = date_trunc( 'week', ?::date ) " .
                                           "        and $dashboard_topic_clause ) q ",
                                           $media_set_b->{ media_sets_id }, $date )->flat;
    ...
    my $words_a = $c->dbis->query( "select $pr_a as stem_count, $pr_b as stem_count_b, " . 
                                   "    coalesce( a.stem, b.stem ) as stem, coalesce( a.term, b.term ) as term " .
                                   "  from " .
                                   "    ( select * from top_500_weekly_words_normalized " . 
                                   "        where media_sets_id = ? and publish_week = date_trunc('week', ?::date ) " . 
                                   "          and $dashboard_topic_clause ) a " .
                                   "    full outer join ".
                                   "    ( select * from top_500_weekly_words_normalized " . 
                                   "        where media_sets_id = ? and publish_week = date_trunc('week', ?::date ) " .
                                   "          and $dashboard_topic_clause ) b " .
                                   "    on ( a.stem = b.stem ) " .
...
sub view : Local
{
    ...
    my $words_query =( "select * from top_500_weekly_words_normalized " . 
                                 ...
                                 "  order by stem_count desc limit " . NUM_WORD_CLOUD_WORDS ); 
