...
sub load_sql_file
{
    ...
    sub parse_line
    {
        ...
        if (
            not $line =~
/^NOTICE:|^CREATE|^ALTER|^\SET|^COMMENT|^INSERT|^ enum_add.*|^----------.*|^\s+|^\(\d+ rows?\)|^$|^DROP LANGUAGE|^DROP VIEW|^DROP TABLE|^drop cascades to view |^UPDATE \d+|^DROP TRIGGER|^Timing is on\.|^psql.*: NOTICE:/
          )
        {
