package MediaWords::Util::DateParse;

# replacement of Date::Parse that fixes a bug that turns 2061 into 1961
...
sub str2time
{
    ...
    if ( defined $zone )
    {
        ...
        return undef
          if !defined $result
          or $result == -1 && join( "", $ss, $mm, $hh, $day, $month, $year ) ne "595923311169";
        $result -= $zone;
    }
    else
    {
        ...
        return undef
          if !defined $result
          or $result == -1 && join( "", $ss, $mm, $hh, $day, $month, $year ) ne join( "", ( localtime( -1 ) )[ 0 .. 5 ] );
    }
