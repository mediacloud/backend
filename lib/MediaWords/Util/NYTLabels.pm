package MediaWords::Util::NYTLabels;
...
# PostgreSQL table name for storing raw NYTLabels annotations
Readonly my $NYTLABELS_POSTGRESQL_KVS_TABLE_NAME => 'nytlabels_annotations';

# Store / fetch downloads using Bzip2 compression
Readonly my $NYTLABELS_USE_BZIP2 => 1;

# (Lazy-initialized) PostgreSQL key-value store shared by ::Util::NYTLabels::* packages
#
...
my $_postgresql_store = lazy
{
    ...
    my $postgresql_store = MediaWords::KeyValueStore::PostgreSQL->new(
        {
            table              => $NYTLABELS_POSTGRESQL_KVS_TABLE_NAME,    #
            compression_method => $compression_method,                     #
        }
    );
    DEBUG "Will read / write NYTLabels annotator results to PostgreSQL table: $NYTLABELS_POSTGRESQL_KVS_TABLE_NAME";

    return $postgresql_store;
};

# Returns true if NYTLabels is enabled
sub nytlabels_is_enabled()
{
...
# Check if story can be annotated with NYTLabels
...
sub story_is_annotatable($$)
{
    ...
    my $story = $db->query(
        <<EOF,
        SELECT story_is_annotatable_with_nytlabels
        FROM story_is_annotatable_with_nytlabels(?)
EOF
        $stories_id
    )->hash;
    if ( $story->{ story_is_annotatable_with_nytlabels } )
    {
        return 1;
    }
    else
...
# Check if story is annotated with NYTLabels
...
# Run the NYTLabels annotation for the story, store results in key-value store
...
sub annotate_and_store_for_story($$)
{
    ...
    {
        WARN "Story $stories_id is already annotated with NYTLabels, so I will overwrite it.";
    }
...
EOF
    ...
    my $annotation = MediaWords::Util::NYTLabels::Annotator::annotate_text( $sentences_concat_text );
    unless ( $annotation )
    {
    ...
    if ( $@ )
    {
        fatal_error( "Unable to store NYTLabels annotation result: $@" );
        return 0;
    }
...
# Fetch the NYTLabels annotation from key-value store for the story
#
...
sub fetch_annotation_for_story($$)
{
    ...
    {
        WARN "Story $stories_id is not annotated with NYTLabels.";
        return undef;
    }
