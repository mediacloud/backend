...
sub lookup_tag 
{
    ...
    return $db->query( "select t.* from tags t, tag_sets ts where t.tag_sets_id = ts.tag_sets_id " .
                       "    and t.tag = ? and ts.name = ?", $tag, $tag_set_name )->hash;
}
...
sub lookup_tag_name
{
    ...
    my ( $tag_set_name, $tag ) = $db->query( "select ts.name, t.tag from tag_sets ts, tags t where ts.tag_sets_id = t.tag_sets_id " .
                                             "    and t.tags_id = ?", $tags_id )->flat;
