...
sub _cache_media_word_vectors 
{
    ...
    my $words = $db->query( "select medium_ms.media_id, mw.stem, min(mw.term) as term, sum(mw.stem_count) as stem_count " . 
                            ...
                            "  group by medium_ms.media_id, mw.stem order by stem_count desc" )->hashes;
...
sub _get_stem_matrix
{
    ...
    my $media = $db->query( 
        ...
        "  where m.media_id = msmm.media_id and msmm.media_sets_id = ? ", $cluster_run->{ media_sets_id } )->hashes;
...
sub execute_and_store_media_cluster_run
{
    ...
    for my $cluster ( @{ $clusters } )
    {
        ...
        my $media_cluster = $db->create( 'media_clusters', { media_cluster_runs_id => $cluster_run->{ media_cluster_runs_id },
                                                             description => $description } );
        
        for my $media_id ( @{ $cluster->{ media_ids } } )
        {
            
            $db->create( 'media_clusters_media_map', { media_clusters_id => $media_cluster->{ media_clusters_id },
                                                       media_id => $media_id } );                                                       
        }
        
        for my $int_feature ( @{ $cluster->{ internal_features } } )
        {
            $db->create( 'media_cluster_words', { media_clusters_id => $media_cluster->{ media_clusters_id },
                                                  internal => 't',
