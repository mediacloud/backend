...
use MediaWords::DB::DatabaseHandler;
...
sub connect_info
{
    ...
    # Arguments for MediaWords::DB::DatabaseHandler->new()
    return (
        $settings->{ host },    #
        $settings->{ port },    #
        $settings->{ user },    #
...
sub connect_to_db(;$$)
{
    ...
    my $ret = MediaWords::DB::DatabaseHandler->new( connect_info( $label ), $do_not_check_schema_version );
