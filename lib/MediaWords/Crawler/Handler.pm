...
sub _add_stories_and_content_downloads
{
    ...
    my $feed = Feed::Scrape->parse_feed( $response->decoded_content ) || die( "Unable to parse feed for $download->{ url }" );
  ...
  ITEM:
    for my $item ( @{$items} )
    {
        ...
        if ( !$story )
        {
            ...
            $story = $dbs->query( "select * from stories where title = ? and media_id = ? " . 
...
sub handle_response
{
    ...
    {
        $dbs->query( "update downloads set state = 'error', error_message = ? where downloads_id = ?",
                     encode( 'utf-8', $response->status_line ), $download->{ downloads_id } );
        return;
    }
